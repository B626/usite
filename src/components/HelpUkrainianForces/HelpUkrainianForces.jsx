...
const HelpUkrainianForces = (props) => {
   const ufOrder = useSelector((state) => state.ufOrder.ufOrder)
   const activeOption = useSelector((state) => state.ufOrder.activeOption)
   const dispatch = useDispatch()
   function onSelectChange(e) {
      dispatch(setActiveOption(e.target.value))
      console.log(e.target.value)
   }
   const Options = ufOrder.map((e, index) => e.location && <option key={index} value={e.location}>{e.location}</option>)
   const Needs = ufOrder.map(e => e.location === activeOption && <h2 className={s.h2}>{e.need} в кількості: {e.amount}</h2>)
   const Prices = ufOrder.map(e => e.location === activeOption && <h2 className={s.h2} >Разом коштують: {e.priceOfUnit * e.amount}грн</h2>)
   const PricesForUnit = ufOrder.map(e => e.location === activeOption && <h2 className={s.h2}>Ціна за одну одиницю: {e.priceOfUnit}грн</h2>)
   return (
      <div className={s.app}>
